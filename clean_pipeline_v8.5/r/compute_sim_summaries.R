...
for (a in 1:(K-1)) {
  for (b in (a+1):K) {
    pi_ratio_vals <- c(pi_ratio_vals, ((het[a] + het[b])/2) / div_flat[length(pi_ratio_vals)+1])
  }
}
...
for (a in 1:(K-1)) {
  for (b in (a+1):K) {
    ...
    pair_fst_vars <- c(pair_fst_vars, stats::var(tapply(fst_site, window_id, mean), na.rm=TRUE))
  }
}
