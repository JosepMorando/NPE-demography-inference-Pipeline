...
from npe_demography.priors import build_theta_keys, is_fixed, get_fixed_value
...
def build_pod_true_params(cfg: dict) -> dict:
    """Build true parameter dict for POD, respecting fixed values from config.

    For fixed parameters: uses the config's fixed value.
    For free parameters: uses sensible mid-range values.
    """
    pri = cfg["priors"]

    # Times — always mid-range (these are always free)
    theta_true = {
        'T_BG01': 500,
        'T_CORE': 800,
        'T_SOUTH_LOW': 1100,
    ...
    # Population sizes — use fixed value if fixed, otherwise mid-range
    size_defaults = {
        'N0': 50000, 'N_BG01': 10000, 'N_CORE': 20000,
        'N_SOUTH_LOW': 15000, 'N_SOUTH_MID': 12000, 'N_EAST': 18000,
        'N_INT': 14000, 'N_CENTRAL': 10000, 'N_PYRENEES': 12000,
    }
    for name, scfg in pri["sizes"].items():
        if is_fixed(scfg):
            theta_true[name] = int(round(get_fixed_value(scfg)))
        else:
            theta_true[name] = size_defaults.get(name, 10000)

    # Demography extras
    extras = pri.get("demography_extras", {})
    if bool(extras.get("enable", False)):
        theta_true['BN_TIME_FRAC'] = 0.4
        theta_true['BN_SIZE_FRAC'] = 0.15
        theta_true['BN_DUR'] = 50

        ex = extras.get("expansion", {})
        if bool(ex.get("enable", True)):
            theta_true['EXP_START_FRAC'] = 0.3
            theta_true['EXP_RATE'] = 0.001
        else:
            theta_true['EXP_START_FRAC'] = 0.0
            theta_true['EXP_RATE'] = 0.0

        mig = extras.get("migration", {})
        if bool(mig.get("enable", False)):
            theta_true['MIG_M'] = 0.0001
            theta_true['MIG_START_FRAC'] = 0.4
        else:
            theta_true['MIG_M'] = 0.0
            theta_true['MIG_START_FRAC'] = 0.0
    else:
        theta_true['BN_TIME_FRAC'] = 0.0
        theta_true['BN_SIZE_FRAC'] = 0.0
        theta_true['BN_DUR'] = 0
        theta_true['EXP_START_FRAC'] = 0.0
        theta_true['EXP_RATE'] = 0.0
        theta_true['MIG_M'] = 0.0
...
def main():
    ...
    # Build true parameters (respects fixed values from config)
    theta_true = build_pod_true_params(cfg)

    # Determine which params are free (will be inferred by NPE)
    theta_keys = build_theta_keys(cfg)

    # Identify fixed params
    pri = cfg["priors"]
